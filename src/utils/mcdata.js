...
const collectblock = collectblockModule.plugin;
...
export function getItemId(itemName) {
    // If mcdata is not initialized yet (before bot login), do basic validation
    if (!mcdata || !mcdata.itemsByName) {
        // Basic validation for common items during bootstrap
        if (typeof itemName === 'string' && itemName.length > 0 && /^[a-z_]+$/.test(itemName)) {
            return 1; // Return a dummy ID to pass validation
        }
        return null;
    }
