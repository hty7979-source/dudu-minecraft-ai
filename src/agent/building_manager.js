...
export class BuildingManager {
    ...
    try {
      ...
      const compatibleSchematic = {
        ...
        forEach: function(callback) {
          console.log(`🔄 Starting forEach iteration over ${width}x${height}x${length} schematic (no rotation)`);
            ...
            for (let z = 0; z < length; z++) {
              for (let x = 0; x < width; x++) {
                ...
                if (index < blockData.length) {
                  ...
                  // ✅ KEINE ROTATION - DIREKTE KOORDINATEN
                  const pos = { 
                    x: x, 
                    y: y, 
                    z: z
  ...
  // ✅ ROTATION ENTFERNT - NICHT MEHR BENÖTIGT
  
  normalizeBlockName(blockName) {
    // ✅ MASSIV VEREINFACHT - NUR CLEANUP FÜR API
    const cleanName = blockName.replace('minecraft:', '');
    const baseName = cleanName.split('[')[0]; // Entferne Block-States
    
    // Nur problematische Blöcke mappen (die in Creative Mode Probleme verursachen)
    const problemBlocks = {
      'wall_torch': 'torch'  // wall_torch funktioniert nicht gut in Creative Mode
    };
    
    const finalBlock = problemBlocks[baseName] || baseName;
  ...
  async buildStructure(schematicName, position = null, rotation = 0) {
    ...
    // ✅ Chat-Nachrichten reduziert - nur wichtigste Infos
  ...
  async startCreativeBuild(schematicInfo, schematicData, position, rotation) {
    ...
    try {
      ...
      if (typeof schematicData.forEach === 'function') {
        ...
        // ✅ GAME MODE HANDLING - CREATIVE VS SURVIVAL
        if (this.bot.game.gameMode === 'creative') {
          console.log(`🎒 Creative Mode: Blocks will be auto-provided`);
          this.bot.chat(`🎨 Creative Mode: All materials available!`);
        } else {
          // ✅ SURVIVAL MODE: Material Check
          console.log(`⚔️ Survival Mode: Checking inventory for required materials...`);
          
          const inventoryCheck = this.analyzeInventoryForBuild(requiredMaterials);
          
          if (!inventoryCheck.canBuild) {
            const missingList = inventoryCheck.missing
              .map(item => `${item.count}x ${item.name}`)
              .join(', ');
            
            this.bot.chat(`❌ Missing: ${missingList}`);
            this.bot.chat(`💡 Please provide materials and try again!`);
            
            // Detaillierte Material-Liste in Console
            console.log('📋 BUILD MATERIAL REQUIREMENTS:');
            Object.entries(requiredMaterials).forEach(([material, needed]) => {
              const have = inventoryCheck.available[material] || 0;
              const status = have >= needed ? '✅' : '❌';
              const missing = Math.max(0, needed - have);
              console.log(`   ${status} ${material}: have ${have}, need ${needed}${missing > 0 ? ` (missing: ${missing})` : ''}`);
            });
            
            return false; // Stop build until materials are provided
          } else {
            console.log(`✅ All materials available in inventory!`);
            this.bot.chat(`✅ Materials ready! Building ${totalBlocks} blocks...`);
          }
        }
        ...
        for (const layerY of sortedLayers) {
          ...
          for (let i = 0; i < layerBlocks.length; i++) {
            ...
            // ✅ SKIP INVENTORY CHECK - CREATIVE MODE AUTO-PROVIDES BLOCKS
            ...
            if (existingBlock && existingBlock.name !== 'air') {
              ...
              if (baseName.includes('bed') && existingBlock.name.includes('bed')) {
                ...
                const baseDelay = settings.block_place_delay || 800;
            ...
            try {
              ...
              try {
                ...
                // ✅ DIREKTE MINEFLAYER API FÜR CREATIVE MODE - KOMPLETT NEU
                const baseName = blockName.split('[')[0];
                const targetPos = new Vec3(Math.floor(worldPos.x), Math.floor(worldPos.y), Math.floor(worldPos.z));
                
                let success = false;
                
                try {
                  // Creative Mode: Verbesserte direkte API-Verwendung
                  if (this.bot.game.gameMode === 'creative') {
                    console.log(`🎯 Creative Mode: Attempting to place ${baseName} at ${targetPos}`);
                    
                    // 1. Spezielle Positionierung für richtungsabhängige Blöcke
                    await this.ensureCorrectPositionForBlock(baseName, targetPos, blockName);
                    
                    // 2. Prüfe ob Block bereits existiert
                    const targetBlock = this.bot.blockAt(targetPos);
                    if (targetBlock.name === baseName) {
                      console.log(`✅ Block ${baseName} already exists at ${targetPos}`);
                      success = true;
                    } else if (targetBlock.name !== 'air') {
                      console.log(`⚠️ Target position occupied by ${targetBlock.name}, clearing first...`);
                      // Hier könnten wir den Block zerstören, aber für jetzt überspringen
                      success = false;
                    } else {
                      // 2. Item direkt erstellen - vereinfachter Ansatz für Creative Mode
                      // Prüfe ob wir bereits das richtige Item haben
                      let hasItem = false;
                      const existingItems = this.bot.inventory.items();
                      for (const item of existingItems) {
                        if (item.name === baseName) {
                          await this.bot.equip(item, 'hand');
                          hasItem = true;
                          console.log(`✅ Found existing ${baseName} in inventory`);
                          break;
                        }
                      }
                      
                      // Falls wir das Item nicht haben, verwende /setblock Command als Fallback
                      if (!hasItem) {
                        console.log(`⚠️ Item ${baseName} not in inventory, using /setblock command`);
                          try {
                            const setblockCmd = `/setblock ${targetPos.x} ${targetPos.y} ${targetPos.z} minecraft:${baseName}`;
                            console.log(`🎮 Fallback executing: ${setblockCmd}`);
                            this.bot.chat(setblockCmd);
                            await new Promise(resolve => setTimeout(resolve, 200));                          // Prüfe Erfolg
                          const checkBlock = this.bot.blockAt(targetPos);
                          if (checkBlock.name === baseName) {
                            console.log(`✅ Successfully placed ${baseName} at ${targetPos} using /setblock`);
                            success = true;
                          } else {
                            console.log(`❌ /setblock failed: found ${checkBlock.name} instead of ${baseName}`);
                            success = false;
                          }
                          // /setblock war erfolgreich, normale Platzierung überspringen
                        } catch (setblockError) {
                          console.log(`❌ /setblock command failed: ${setblockError.message}`);
                          success = false;
                        }
                      }
                      
                      // 3. Normale Block-Platzierung nur wenn wir ein Item haben und success noch false ist
                      if (hasItem && !success) {
                        let placed = false;
                        // Referenzblock für Platzierung finden
                        const directions = [
                          new Vec3(0, -1, 0),  // unten
                          new Vec3(0, 1, 0),   // oben
                          new Vec3(1, 0, 0),   // ost
                          new Vec3(-1, 0, 0),  // west
                          new Vec3(0, 0, 1),   // süd
                        ...
                        for (const direction of directions) {
                          const refPos = targetPos.plus(direction);
                          const refBlock = this.bot.blockAt(refPos);
                          
                          if (refBlock && refBlock.name !== 'air') {
                            try {
                              const faceVector = new Vec3(-direction.x, -direction.y, -direction.z);
                              ...
                              const checkBlock = this.bot.blockAt(targetPos);
                              if (checkBlock.name === baseName) {
                                console.log(`✅ Successfully placed ${baseName} at ${targetPos} using reference ${refBlock.name}`);
                                placed = true;
                                success = true;
                                break;
                              }
                            } catch (placeError) {
                              console.log(`❌ Failed to place on ${refBlock.name}: ${placeError.message}`);
                              continue;
                            }
                          }
                        }
                        ...
                        if (!placed) {
                          console.log(`⚠️ No reference block found, trying /setblock fallback for ${baseName} at ${targetPos}`);
                          try {
                            const setblockCmd = `/setblock ${targetPos.x} ${targetPos.y} ${targetPos.z} minecraft:${baseName}`;
                            console.log(`🎮 Executing: ${setblockCmd}`);
                            this.bot.chat(setblockCmd);
                            ...
                            const checkBlock = this.bot.blockAt(targetPos);
                            if (checkBlock.name === baseName) {
                              console.log(`✅ Successfully placed ${baseName} at ${targetPos} using /setblock fallback`);
                              success = true;
                            } else {
                              console.log(`❌ /setblock fallback failed: found ${checkBlock.name} instead of ${baseName}`);
                              success = false;
                            }
                          } catch (setblockError) {
                            console.log(`❌ /setblock fallback command failed: ${setblockError.message}`);
                            success = false;
                          }
                        }
                      }
                    ...
                    // Survival Mode: Verwende bestehende Skills API
                    console.log(`⚔️ Survival Mode: Using skills.placeBlock for ${baseName}`);
                    success = await skills.placeBlock(this.bot, baseName, worldPos.x, worldPos.y, worldPos.z, 'bottom', false);
                  }
                } catch (error) {
                  console.log(`❌ Block placement error for ${baseName}: ${error.message}`);
                  success = false;
                }
                ...
                if (success) {
                  ...
                  const currentBlock = this.bot.blockAt(targetVec);
                  ...
                  // Zeige Inventar für Debug
                  const items = this.bot.inventory.items();
                  console.log(`🎒 Current inventory: ${items.map(i => `${i.name}:${i.count}`).join(', ')}`);
                }
              } catch (placeError) {
                errors++;
  ...
  // ✅ INVENTORY MANAGEMENT ENTFERNT - CREATIVE MODE HANDLED AUTOMATICALLY
  ...
  /**
   * 🎯 Stellt sicher, dass der Bot für richtungsabhängige Blöcke korrekt positioniert ist
   */
  async ensureCorrectPositionForBlock(baseName, targetPos, fullBlockName) {
    // Definiere Blöcke die spezielle Positionierung benötigen
    const directionalBlocks = {
      // Betten - Bot muss am Fußende stehen
      'bed': { 
        type: 'bed', 
        needsPosition: true, 
        description: 'Bed placement requires correct facing direction' 
      },
      
      // Öfen - Bot sollte von vorne platzieren
      'furnace': { 
        type: 'furnace', 
        needsPosition: true, 
        description: 'Furnace should face the player' 
      },
      'blast_furnace': { 
        type: 'furnace', 
        needsPosition: true, 
        description: 'Blast furnace should face the player' 
      },
      'smoker': { 
        type: 'furnace', 
        needsPosition: true, 
        description: 'Smoker should face the player' 
      },
      
      // Kisten - Bot sollte von vorne platzieren
      'chest': { 
        type: 'chest', 
        needsPosition: true, 
        description: 'Chest should face the player' 
      },
      'trapped_chest': { 
        type: 'chest', 
        needsPosition: true, 
        description: 'Trapped chest should face the player' 
      },
      
      // Türen - Bot muss korrekt stehen für Öffnungsrichtung
      'door': { 
        type: 'door', 
        needsPosition: true, 
        description: 'Door hinge placement depends on player position' 
      },
      
      // Treppen - Richtung ist wichtig
      'stairs': { 
        type: 'stairs', 
        needsPosition: true, 
        description: 'Stairs direction based on player facing' 
      },
      
      // Wiederhol-Blöcke (Repeater) - Richtung wichtig  
      'repeater': { 
        type: 'redstone', 
        needsPosition: true, 
        description: 'Repeater direction based on player facing' 
      },
      'comparator': { 
        type: 'redstone', 
        needsPosition: true, 
        description: 'Comparator direction based on player facing' 
      },
      
      // Leitern - müssen an Wand platziert werden
      'ladder': { 
        type: 'ladder', 
        needsPosition: true, 
        description: 'Ladder must be placed against wall' 
      }
    };

    // Prüfe ob Block spezielle Behandlung braucht
    let blockInfo = null;
    for (const [key, info] of Object.entries(directionalBlocks)) {
    ...
    if (!blockInfo || !blockInfo.needsPosition) {
      return; // Kein spezieller Block, normale Platzierung
    }

    console.log(`🎯 Positioning bot for directional block: ${baseName} (${blockInfo.description})`);

    try {
      // Extrahiere Richtungsinformation aus Block States falls vorhanden
      let targetFacing = null;
      if (fullBlockName.includes('[facing=')) {
        const match = fullBlockName.match(/facing=([a-z]+)/);
        if (match) {
          targetFacing = match[1];
          console.log(`🧭 Block specifies facing direction: ${targetFacing}`);
        }
      }

      // Berechne optimale Bot-Position basierend auf Block-Typ
      let optimalBotPos = null;
      let targetYaw = null;

      switch (blockInfo.type) {
        case 'bed':
          // Für Betten: Bot sollte am Fußende stehen und zum Kopfende schauen
          if (targetFacing) {
            optimalBotPos = this.calculateBedPlacementPosition(targetPos, targetFacing);
            targetYaw = this.facingToYaw(targetFacing);
          }
          break;
          
        case 'furnace':
        case 'chest':
          // Für Öfen/Kisten: Bot sollte so stehen, dass die Front zum Bot zeigt
          if (targetFacing) {
            optimalBotPos = this.calculateFrontFacingPosition(targetPos, targetFacing);
            targetYaw = this.facingToYaw(this.getOppositeFacing(targetFacing));
          } else {
            // Fallback: Bot sollte südlich vom Block stehen und nach Norden schauen
            optimalBotPos = new Vec3(targetPos.x, targetPos.y, targetPos.z + 2);
            targetYaw = 0; // Nord
          }
          break;
          
        case 'door':
          // Für Türen: Position beeinflusst Scharnier-Seite
          if (targetFacing) {
            optimalBotPos = this.calculateDoorPlacementPosition(targetPos, targetFacing, fullBlockName);
            targetYaw = this.facingToYaw(targetFacing);
          }
          break;
        ...
        case 'stairs':
        case 'redstone':
          // Für Treppen/Redstone: Bot schaut in gewünschte Richtung
          if (targetFacing) {
            optimalBotPos = new Vec3(targetPos.x, targetPos.y, targetPos.z);
            targetYaw = this.facingToYaw(targetFacing);
          }
          break;
          
        case 'ladder':
          // Für Leitern: Bot steht vor der Wand
          if (targetFacing) {
            optimalBotPos = this.calculateLadderPlacementPosition(targetPos, targetFacing);
            targetYaw = this.facingToYaw(this.getOppositeFacing(targetFacing));
          }
          break;
      }

      // Bewege Bot zur optimalen Position falls berechnet
      if (optimalBotPos && this.bot.entity.position.distanceTo(optimalBotPos) > 1.5) {
        console.log(`🚶 Moving bot to optimal position for ${baseName}: ${optimalBotPos}`);
        
        // Verwende Pathfinder für präzise Bewegung
        const goal = new goals.GoalNear(optimalBotPos.x, optimalBotPos.y, optimalBotPos.z, 1);
        this.bot.pathfinder.setGoal(goal);
        
        // Warte bis Bot angekommen ist (max 3 Sekunden)
      ...
      // Richte Bot-Blickrichtung aus falls berechnet
      if (targetYaw !== null) {
        console.log(`🧭 Setting bot yaw to ${targetYaw}° for ${baseName}`);
        await this.bot.look(targetYaw * Math.PI / 180, 0, true); // Konvertiere zu Radians
        await new Promise(resolve => setTimeout(resolve, 200)); // Kurz warten
      }

      console.log(`✅ Bot positioned correctly for ${baseName} placement`);

    } catch (error) {
      console.log(`⚠️ Could not optimize bot position for ${baseName}: ${error.message}`);
      // Nicht kritisch - normale Platzierung wird versucht
    }
  }

  /**
   * Hilfsmethoden für Positionsberechnungen
   */
  calculateBedPlacementPosition(targetPos, facing) {
    // Bot steht am Fußende des Bettes
    const facingOffsets = {
      'north': new Vec3(0, 0, 1),    // Bot südlich vom Bett
      'south': new Vec3(0, 0, -1),   // Bot nördlich vom Bett  
      'east': new Vec3(-1, 0, 0),    // Bot westlich vom Bett
      'west': new Vec3(1, 0, 0)      // Bot östlich vom Bett
    };
    return targetPos.plus(facingOffsets[facing] || new Vec3(0, 0, 1));
  }

  calculateFrontFacingPosition(targetPos, facing) {
    // Bot steht vor der Front des Blocks
    const facingOffsets = {
      'north': new Vec3(0, 0, -2),   // Bot nördlich vom Block
      'south': new Vec3(0, 0, 2),    // Bot südlich vom Block
      'east': new Vec3(2, 0, 0),     // Bot östlich vom Block
      'west': new Vec3(-2, 0, 0)     // Bot westlich vom Block
    };
    return targetPos.plus(facingOffsets[facing] || new Vec3(0, 0, 2));
  }

  calculateDoorPlacementPosition(targetPos, facing, fullBlockName) {
    // Für Türen: Scharnier-Position hängt von Bot-Position ab
    const isRightHinge = fullBlockName.includes('hinge=right');
    let offset = new Vec3(0, 0, 1); // Standard: Bot südlich
    
    if (facing === 'north') offset = new Vec3(isRightHinge ? 1 : -1, 0, 1);
    else if (facing === 'south') offset = new Vec3(isRightHinge ? -1 : 1, 0, -1);
    else if (facing === 'east') offset = new Vec3(-1, 0, isRightHinge ? 1 : -1);
    else if (facing === 'west') offset = new Vec3(1, 0, isRightHinge ? -1 : 1);
    
    return targetPos.plus(offset);
  }

  calculateLadderPlacementPosition(targetPos, facing) {
    // Bot steht vor der Leiter (gegenüber der Wand)
    const facingOffsets = {
      'north': new Vec3(0, 0, -2),   // Leiter an Nord-Wand, Bot im Süden
      'south': new Vec3(0, 0, 2),    // Leiter an Süd-Wand, Bot im Norden
      'east': new Vec3(2, 0, 0),     // Leiter an Ost-Wand, Bot im Westen
      'west': new Vec3(-2, 0, 0)     // Leiter an West-Wand, Bot im Osten
    };
    return targetPos.plus(facingOffsets[facing] || new Vec3(0, 0, 2));
  }

  facingToYaw(facing) {
    // Konvertiere Minecraft Facing zu Yaw in Grad
    const facingYaw = {
      'north': 0,     // 0° = Nord
      'east': 90,     // 90° = Ost  
      'south': 180,   // 180° = Süd
  ...
  getOppositeFacing(facing) {
    ...
    return opposites[facing] || 'north';
  }

  /**
   * 📦 Analysiert Inventar für Survival Mode Builds
   */
  analyzeInventoryForBuild(requiredMaterials) {
    const available = {};
    const missing = [];
    const surplus = [];
    
    // Prüfe jedes benötigte Material
    for (const [material, needed] of Object.entries(requiredMaterials)) {
      const baseName = material.split('[')[0]; // Entferne Block States
      const have = this.countItemInInventory(baseName);
      available[baseName] = have;
    ...
    return {
      ...
      missing: missing,
      surplus: surplus,
      totalRequired: Object.values(requiredMaterials).reduce((sum, count) => sum + count, 0),
      totalAvailable: Object.values(available).reduce((sum, count) => sum + count, 0)
    };
  }

  /**
   * 📋 Zeigt detaillierte Material-Anforderungen im Chat
   */
  showMaterialRequirements(requiredMaterials) {
    this.bot.chat('📋 MATERIAL REQUIREMENTS:');
    
    const materialList = Object.entries(requiredMaterials)
      .sort(([,a], [,b]) => b - a) // Sortiere nach Menge
      .slice(0, 10) // Zeige max 10 wichtigste
      .map(([material, count]) => `${count}x ${material}`)
      .join(', ');
    
    this.bot.chat(`📦 ${materialList}`);
    
    // Zeige Inventar-Status
    const inventoryCheck = this.analyzeInventoryForBuild(requiredMaterials);
    
    if (inventoryCheck.canBuild) {
      this.bot.chat('✅ All materials available in inventory!');
    } else {
      const missingCount = inventoryCheck.missing.length;
      const totalMissing = inventoryCheck.missing.reduce((sum, item) => sum + item.count, 0);
      this.bot.chat(`❌ Missing ${totalMissing} items (${missingCount} types)`);
    }
  }
}
